...
def write_xyz_file(coords, atom_types, filename):
    ...
    # newline='' ОБЯЗАТЕЛЕН на Windows: без него open(..., 'w') молча
    # переводит каждый \n в \r\n (универсальные переводы строк текстового
    # режима), а парсер XYZ у OpenBabel (написан только под Linux) не
    # переваривает \r в потоке — путает границы строк/атомов и валится с
    # "Cannot convert 'X -' to unsigned int" на КАЖДОЙ сгенерированной
    # молекуле (проверено: сравнение сырых байтов файла показало 0d0a
    # вместо 0a на каждой строке). На Linux newline='' не меняет
    # поведение (там \n и так остаётся \n).
